...
def _raise_if_vram_over_threshold(
    ...
    if peak_mb > threshold_mb:
        raise TrainingGuardrailViolation(
            f"peak VRAM {peak_mb:.0f} MB exceeded threshold "
            f"{threshold_mb:.0f} MB at {label}"
        )
...
def train_cascades(
    ...
    Args:
        ...
        abort_on_nonfinite: Opt-in guardrail that aborts on non-finite train/eval loss.
        vram_threshold_mb: Optional opt-in hard peak VRAM threshold in MB.
    ...
    _raise_if_vram_over_threshold(vram_threshold_mb, device, "after_model_load")
    ...
    if config.enable_dmole_select:
        ...
        _raise_if_vram_over_threshold(vram_threshold_mb, device, "after_dmole_probe")
    ...
    _raise_if_vram_over_threshold(vram_threshold_mb, device, "after_adapter_injection")
    reset_peak_stats(device)
    ...
    for t in range(num_tasks):
            ...
            for batch in dataloader:
                ...
                if abort_on_nonfinite:
                    _raise_if_nonfinite_tensor(
                        loss,
                        f"training loss task={t} epoch={ep + 1} batch={num_batches + 1}",
                    )
                loss.backward()
                ...
                num_batches += 1
                _raise_if_vram_over_threshold(
                    vram_threshold_mb,
                    device,
                    f"task={t} epoch={ep + 1} batch={num_batches}",
                )
                ...
                if ep == 0 and num_batches == 1:
                    ...
                    check_oom_risk(threshold_mb=vram_threshold_mb or 7500.0, device=device)
                    _raise_if_vram_over_threshold(
                        vram_threshold_mb,
                        device,
                        f"after_first_backward task={t} epoch={ep + 1} batch={num_batches}",
                    )
                ...
                if enable_sleep and num_batches % 100 == 0 and num_batches > 0:
                    ...
                    _raise_if_vram_over_threshold(
                        vram_threshold_mb,
                        device,
                        f"after_micro_sleep task={t} epoch={ep + 1} batch={num_batches}",
                    )
        ...
        if enable_sleep:
            ...
            _raise_if_vram_over_threshold(vram_threshold_mb, device, f"after_sleep task={t}")

        # --- Evaluation ---
        clear_cache("before_eval", device)
        _raise_if_vram_over_threshold(vram_threshold_mb, device, f"before_eval task={t}")
            ...
            for eval_t in range(t + 1):
                ...
                for batch in eval_loader:
                    ...
                    if abort_on_nonfinite:
                        _raise_if_nonfinite_tensor(
                            out.loss,
                            f"eval loss train_task={t} eval_task={eval_t} batch={n_batches + 1}",
                        )
                ...
                if abort_on_nonfinite and not math.isfinite(avg_loss):
                    raise TrainingGuardrailViolation(
                        f"non-finite average eval loss train_task={t} eval_task={eval_t}"
                    )
        ...
        _raise_if_vram_over_threshold(vram_threshold_mb, device, f"after_eval task={t}")
