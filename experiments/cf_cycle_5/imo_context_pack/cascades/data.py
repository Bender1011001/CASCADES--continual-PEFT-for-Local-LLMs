...
TASK_FILES: list[str] = [
    ...
    "data/task3_digital_twin.jsonl",  # personal identity data — trains AFTER all reasoning tasks
]
...
def _build_supervised_sequence(
    ...
    """Build a truncated causal-LM sample that always preserves response labels."""
    if max_length < 1:
        raise ValueError("max_length must be at least 1")
    if not response_tokens:
        raise ValueError("response must tokenize to at least one supervised token")

    if len(prompt_tokens) + len(response_tokens) > max_length:
        prompt_budget = min(len(prompt_tokens), max_length - 1)
        response_budget = max_length - prompt_budget
        prompt_tokens = prompt_tokens[-prompt_budget:] if prompt_budget else []
    ...
    if not any(label != -100 for label in labels):
        raise ValueError("truncation produced zero supervised response labels")
...
def prepare_data(
    ...
    for prompt, response in zip(df["prompt"], df["response"]):
        ...
        input_ids, attention_mask, labels = _build_supervised_sequence(
            prompt_tokens, response_tokens, max_length
        )
