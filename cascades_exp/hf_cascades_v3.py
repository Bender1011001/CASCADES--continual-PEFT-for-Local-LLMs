...
def deal_heat_kernel_filter(grad, quant_noise_std=0.0, t=0.05, lambda_decay=0.01):
    """DEAL arXiv'25: wavelet heat-kernel low-pass filter on gradients.
    v3 fix: adaptive threshold ε_quant based on quantization noise floor.
    Singular values below ε_quant are zeroed (noise); above are filtered by heat kernel."""
    if not ENABLE_DEAL or grad.dim() < 2:
        return grad
    u, s, v = torch.svd(grad.float())
    # Quantization-aware threshold (v3 fix for v2 over-damping)
    eps_quant = max(1e-4, quant_noise_std * 0.1)
    # Keep signal above noise floor, apply heat kernel decay to surviving components
...
def cllora_gradient_reassign(grad, null_sketch, alpha=0.5):
    """CL-LoRA CVPR'25: redirect blocked gradient energy into the free subspace.
    Instead of simply zeroing the occupied-subspace component, we reassign its magnitude
    to the dominant direction of the free subspace. Preserves ||∇||_F."""
    ...
    # Find dominant free direction via the null component itself
    occupied_energy = occupied_component.norm()
    if occupied_energy > 1e-8 and null_component.norm() > 1e-8:
        free_direction = null_component / null_component.norm()
        reassigned = null_component + alpha * occupied_energy * free_direction
        return reassigned
...
def compute_layer_importance(model, dataloader, device, threshold=0.15):
    ...
    # Single batch for importance estimation
    with torch.no_grad():
