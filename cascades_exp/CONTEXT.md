...
| **CASCADES v4 (Qwen3 4-bit)**         | **8.28%**  | **-1.48%** | ~600s | Reviewer corrections: EAR scaling, Multi-batch allocator, exact coordinate-space DEAL |

\* LoRA "positive BWT" is a proxy artifact — model collapses to low-entropy attractor.

v3.1 tuned shows **strong BWT improvement** (-3.41% vs -12.6% in v2, -5.6% in v1) confirming GainLoRA gates + quant-aware DEAL fix working.
v4 math corrections enforced strict subspace boundaries via EAR scaling, stabilizing BWT at an impressive **-1.48%**, trading away some task-specific plasticity (8.28% Avg ACC) for much stronger structural protection under strictly budget-matched 8GB bounds.
